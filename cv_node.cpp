...
cv::Mat image_processing(const cv::Mat in_image) 
{
  ...
  // Processing
  out_image = in_image; 
  std::string text = ("1: RGB, 2: CMY, 3: HSI, 4: HSV, 5: HSV OpenCV, 6: HSI OpenCv");
  cv::putText(out_image, text, cv::Point(10, 20), cv::FONT_HERSHEY_SIMPLEX, 0.5, cv::Scalar(0, 0, 255));
  ...
  {// Option 4 --- HSV pix2pix.
    ...
    cv::imshow("HSV pixel",out_image);
    return out_image;
  }
  else if (c == 53 || c == -75)
  { // Option 5 -- HSV OpenCV.
    cvtColor(in_image, out_image, CV_BGR2HSV);
    cv::imshow("HSV",out_image);
    return out_image;
  }
  ...
  { // Option 6.
    ...
    std::vector<cv::Mat> three_channels_BGR; // canales BGR
    cv::split( temp_image, three_channels_BGR );

    std::vector<cv::Mat> three_channels; // canales HSI
    cv::split( temp_image, three_channels );
    
    // Convertimos de BGR a CMY haciendo la conversión de valores.

    std::vector<cv::Mat> channels;
    channels.push_back(three_channels[0]);
    channels.push_back(three_channels[1]);
    channels.push_back((three_channels_BGR[0]+three_channels_BGR[1]+three_channels_BGR[2])/3);

    merge(channels, out_image);
    cv::imshow("HSI",out_image);
    return out_image;
